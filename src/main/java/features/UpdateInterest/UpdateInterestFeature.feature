...
Feature: Create Interest
  ...
  Scenario Outline: Validate Updating Interest Using Invalid URL/Method
    Given I want to Update interest
     When I update interest using url: "<url>" and method "<method>"
     Then I verify the actual results: Expected: code "<statusCode>", message "<message>"

    Examples: 
      | url                                        | method | statusCode | message            |
      | http://localhost:8080/api/v1/interested/12 | PUT    |        404 | Not Found          |
      | http://localhost:8080/api/v1/interest/12   | GET    |        405 | Method Not Allowed |

  @CheckInvalidToken
  Scenario: Validate Updating Interest Using Invalid Token
  ...
  @ValidateFields
  Scenario Outline: Validate Updating Interest Fields
    Given I want to update an interest.
     When I update interest using: key "<key>" with value "<value>"
